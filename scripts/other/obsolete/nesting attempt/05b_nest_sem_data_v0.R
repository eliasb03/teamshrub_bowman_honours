...
nesting_data <- read_csv(file.path(directory, nest.file))
nest_data <- nesting_data %>%
  filter(phase = "nest")
egg_data <- nesting_data %>%
  filter(phase = "egg")
...
join_by_year <- function(survey_data, ice_data, phenology_data, climate_data, 
                         survey_cols, ice_cols, phenology_cols, climate_cols) {
  # Select the specified columns from each dataset
  survey_selected <- survey_data %>% select(all_of(survey_cols))
...
final_data <- join_by_year(
  survey_data = bbs_data,
  ice_data = ice_data,
  phenology_data = phenology_data,
  climate_data = climate_data,
  survey_cols = bbs_cols,
  ice_cols = ice_cols,
...
write_csv(scaled_data, "data/clean/sem/bbs_sem_data.csv")
write_csv(scaling_params, "data/clean/sem/bbs_scaling_params.csv")

###

nest_cols <- c("year", "spec.code", "species", "guild", "rel.abundance.total", "rel.abundance.scaled", "logistic.id.total", "total.count")

egg_cols <- c("year", "spec.code", "species", "guild", "rel.abundance.total", "rel.abundance.scaled", "logistic.id.total", "total.count")

nest_final_data <- join_by_year(
  survey_data = nest_data,
  ice_data = ice_data,
  phenology_data = phenology_data,
  climate_data = climate_data,
  survey_cols = nest_cols,
  ice_cols = ice_cols,
  phenology_cols = phenology_cols,
  climate_cols = climate_cols
) 

egg_final_data <- join_by_year(
  survey_data = gg_data,
  ice_data = ice_data,
  phenology_data = phenology_data,
  climate_data = climate_data,
  survey_cols = egg_cols,
  ice_cols = ice_cols,
  phenology_cols = phenology_cols,
  climate_cols = climate_cols
) 
...
column_mapping <- c(
  ...
  "spring_drop_doy" = "ice.melt",
  #"rel.abundance.scaled" = "bird.abundance"
  "total.count" = "nest.date",
  "total.count" = "egg.date",
)

nest_final_data <- nest_final_data %>%
  select( # Select relevant columns
    year,
    species,
    guild,
