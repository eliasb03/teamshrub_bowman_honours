...
#
# Description: This script imports, tidies, and processes ARU data into observance records from Qikiqtaruk - Herschel Island. It processes all ARU audio files in the given directory using BirdNET's batch analysis and outputs a formatted CSV for the entire dataset.
...
aru_dir <- "D:/ARU_QHI_2024/ARUQ4_20Aug2024"
...
#------------------------------
# ARU Processing Functions ####
#------------------------------

# Function to run BirdNET analysis in batch mode on all files in the directory
process_aru_files <- function(aru_dir) {
  data_dir <- file.path(aru_dir, "Data")  # Directory containing .wav files
  output_dir <- file.path(aru_dir, "Output")
  
  check_and_create_dir(output_dir)  # Create output directory if it doesn't exist
  
  cat("Starting BirdNET batch analysis on all .wav files in", data_dir, "...\n")
  
  cat("Outputs will be saved to the directory:", output_dir, "\n")
  
  # Run BirdNET batch analysis
  ...
  # Save the gathered results to a CSV file
  save_combined_results(gathered_results, aru_dir)
  
  cat("Finished processing all files in", aru_dir, "...\n")
...
run_aru_pipeline_single_folder <- function(aru_dir) {
  ...
  pipe.start.t <- Sys.time()
  cat("Starting at", format(pipe.start.t, "%H:%M:%S", tz = qhi_timezone), "\n")
  
  combined_results <- process_aru_files(aru_dir)
...
# # Global Variables ####
# # Set your specific ARU directory here
# aru_dir <- "D:/ARU_QHI_2024/ARUQ3_20Aug2024"
# 
# # Field site specific variables
# qhi_timezone <- "America/Dawson"
# 
...
# #   
# #   cat(paste0("Reading or creating log file ", log_path, "...\n"))
